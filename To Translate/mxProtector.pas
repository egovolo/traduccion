     ...
     Begin
          ...
          Begin
              ...
              dm.QArchivo.SQL.Text    :=    ' delete from ARCHIVOFS  '+
                                            ' WHERE  NOMBRE=UPPER(''oorntlc3'')' ;
               dm.QArchivo.ExecSQL;


          

             vtexto:= inttostr(Year)+inttostr(Month)+inttostr(Day);
          ...
          Begin
               ...
               Begin
                   ...
                   If not FileExists(vPath) then   Begin
                    ...
                    if ValidSystemTime then begin //   if ValidSystemTime then begin//20100102
                             DateStr := Get_String( 'D2', CodePassword( IntToHex( Year, 4 ) + IntToHex( Month, 2 ) + IntToHex( Day, 2 ), True ) );
                             vtexto := inttostr(Year)+inttostr(Month)+inttostr(Day);
                             ...
                             DM.QArchivo.Open;
                             if (DM.QArchivo.fieldbyname('cuantos').asinteger = 0) and  (LastStart > Now) then Begin
                               If Assigned( FOnInvalidSystemTime ) Then FOnInvalidSystemTime( Self );
                                  ValidSystemTime := false;
                                   If Assigned( FOnExpiration ) Then FOnExpiration( Self );
                             ...
                             End;   //if DM.QArchivo.fieldbyname('cuantos').asinteger = 0 then Begin
                  End;//   if ValidSystemTime then begin//20100102
