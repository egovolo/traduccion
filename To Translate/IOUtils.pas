...
{$IFDEF MSWINDOWS}
    class function ConvertFileTimeToDateTime(const FileTime: TFileTime;
        const UseLocalTimeZone: Boolean): TDateTime; static;
    class function ConvertDateTimeToFileTime(const DateTime: TDateTime;
        const UseLocalTimeZone: Boolean): TFileTime; static;
{$ENDIF}
{$IFDEF POSIX}
    class function ConvertFileTimeToDateTime(const FileTime: time_t;
        const UseLocalTimeZone: Boolean): TDateTime; static;
    class function ConvertDateTimeToFileTime(const DateTime: TDateTime;
...
{$ENDIF MSWINDOWS}
    ...
    class function DoMatchesPattern(const FileName, Pattern: string): Boolean; static;
  public
...
uses
{$IFDEF POSIX}
  PosixBase, PosixStdio, PosixStdlib, PosixUnistd, PosixSysStat, PosixTime,
  PosixUtime,
{$ENDIF}
  StrUtils, DateUtils, Masks, Math, RTLConsts;
...
{$IFDEF MSWINDOWS}
class function TDirectory.ConvertFileTimeToDateTime(
  const FileTime: TFileTime; const UseLocalTimeZone: Boolean): TDateTime;
var
  LFileTime: TFileTime;
  SysTime: TSystemTime;
begin
...
{$IFDEF POSIX}
class function TDirectory.ConvertFileTimeToDateTime(
  const FileTime: time_t; const UseLocalTimeZone: Boolean): TDateTime;
begin
    ...
    PostCallback :=
      ...
      begin
        ...
        begin
          ...
          // the deletion
{$IFDEF MSWINDOWS}
          FileSetAttr(CompletePath, SysUtils.faNormal);
{$ENDIF MSWINDOWS}

          case FileInfo.Attr and SysUtils.faDirectory of
            SysUtils.faDirectory: // remove empty directories
    ...
    LastDirInfo.Attr := SysUtils.faDirectory;
    PostCallback(GetParent(Path), LastDirInfo);
  end
  else
  begin
{$IFDEF MSWINDOWS}
    FileSetAttr(Path, SysUtils.faNormal);
{$ENDIF}
    RemoveDir(Path);
  end;
end;
...
{$IFDEF MSWINDOWS}
              FileSetAttr(CompleteSrc, SysUtils.faNormal);
{$ENDIF}
...
{$IFDEF MSWINDOWS}
    FileSetAttr(SourceDirName, SysUtils.faDirectory);
{$ENDIF MSWINDOWS}
...
begin
  ...
  try
    try
      ...
      if LFileEncoding = TEncoding.Default then
      begin
        ...
        begin
          LFileStream.Seek(0, TSeekOrigin.soEnd);
          Buff := TEncoding.Default.GetBytes(Contents);
        end
...
{$ENDIF}
  ...
  case Mode of
    ...
    TFileMode.fmCreate:
      try
        Result := TFileStream.Create(Path, Classes.fmCreate, LFileStrmShare);
      except
        on E: EFileStreamError do
          raise EInOutError.Create(E.Message);
      end;
...
{$IFDEF POSIX}
begin
  { Re-route to SysUtils.ExtractFileDir for POSIX }
...
begin
  if Pattern = '*.*' then
    Result := MatchesMask(FileName, '*') // DO NOT LOCALIZE
  else
...
begin
  ...
  if Windows.GetTempPath(Len, PWideChar(Result)) <> 0 then
  begin
