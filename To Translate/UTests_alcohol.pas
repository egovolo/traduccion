...
type
  TFTests_alcohol = class(TForm)
    ...
    RadioGroup81: TRadioGroup;
    RadioGroup82: TRadioGroup;
    Memo10: TMemo;
    Memo11: TMemo;
    Memo12: TMemo;
    Memo13: TMemo;
...
var
  FTests_alcohol: TFTests_alcohol;
...
procedure TFTests_alcohol.Button100Click(Sender: TObject);
begin
 close;
end;

procedure TFTests_alcohol.Button84Click(Sender: TObject);
begin

CLOSE;
end;

procedure TFTests_alcohol.Button86Click(Sender: TObject);
begin
   pCalcula_Test(20)
end;

procedure TFTests_alcohol.Button87Click(Sender: TObject);
begin
  memo1.BringToFront;
  Memo1.Visible:=True;
end;

procedure TFTests_alcohol.Button90Click(Sender: TObject);
begin
     pCalcula_Test(21)
end;

procedure TFTests_alcohol.Button91Click(Sender: TObject);
begin
 Memo5.BringToFront;
 Memo5.Visible  := True;
end;

procedure TFTests_alcohol.Button92Click(Sender: TObject);
begin
  close;
end;

procedure TFTests_alcohol.Button94Click(Sender: TObject);
begin
    pCalcula_Test(22)
end;

procedure TFTests_alcohol.Button95Click(Sender: TObject);
begin
 Memo10.BringToFront;
 Memo10.Visible:=True;
end;

procedure TFTests_alcohol.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FTests_alcohol := nil;
  FTests_alcohol.free;
    self.destroying;
...
procedure TFTests_alcohol.FormCreate(Sender: TObject);
begin
 //ReplaceRadioGroups( FTests_alcohol) ;
end;

procedure TFTests_alcohol.FormShow(Sender: TObject);
 var
  VPath :String;
begin
ReplaceRadioGroups( FTests_alcohol) ;
...
procedure TFTests_alcohol.Memo10DblClick(Sender: TObject);
begin
  Memo10.SendToBack;
  Memo10.Visible:=False;
end;

procedure TFTests_alcohol.Memo11DblClick(Sender: TObject);
begin
 memo11.SendToBack;
 Memo11.Visible:=False;
end;

procedure TFTests_alcohol.Memo12DblClick(Sender: TObject);
begin
  memo12.SendToBack;
  Memo12.Visible:=False;
end;

procedure TFTests_alcohol.Memo13DblClick(Sender: TObject);
begin
 Memo13.SendToBack;
 Memo13.Visible:=False;
end;

procedure TFTests_alcohol.Memo1DblClick(Sender: TObject);
begin
   Memo1.SendToBack;
   Memo1.Visible:=False;
end;

procedure TFTests_alcohol.Memo2DblClick(Sender: TObject);
begin
  memo2.SendToBack;
  Memo2.Visible:=False;
end;

procedure TFTests_alcohol.Memo3DblClick(Sender: TObject);
begin
   memo3.SendToBack;
   Memo3.Visible:=False;
end;

procedure TFTests_alcohol.Memo4DblClick(Sender: TObject);
begin
   memo4.SendToBack;
   Memo4.Visible:=False;
end;

procedure TFTests_alcohol.Memo5DblClick(Sender: TObject);
begin
 Memo5.SendToBack;
 Memo5.Visible:=False;
end;

procedure TFTests_alcohol.Memo6DblClick(Sender: TObject);
begin
  Memo6.SendToBack;
  Memo6.Visible:=False;
end;

procedure TFTests_alcohol.Memo7DblClick(Sender: TObject);
begin
  Memo7.SendToBack;
  Memo7.Visible:=False;
end;

procedure TFTests_alcohol.Memo8DblClick(Sender: TObject);
begin
   Memo8.SendToBack;
   Memo8.Visible:=False;
end;

procedure TFTests_alcohol.pCalcula_Test(ptag : Integer);
var
  vX ,
  vz   : Integer;
...
Begin
 ...
 vBChecadoTotal := False;
  ...
  for vX :=0 to  FTests_alcohol.ComponentCount -1 do Begin
        if  (FTests_alcohol.Components[vX] is TMyRadioGroup)And
             (FTests_alcohol.Components[vX].tag = ptag)  then begin
                 vBChecadoTotal  :=False;
                 if not (FTests_alcohol.Components[vX] as TMyRadioGroup).visible then
                    vBChecadoTotal  := True;
                 if (FTests_alcohol.Components[vX] as TMyRadioGroup).itemindex <> -1 then
                          vBChecadoTotal := True;

                 if not vBChecadoTotal Then Begin
                    Showmessage('Por favor complete todo el test antes de proceder. '+(FTests_alcohol.Components[vX] as TMyRadioGroup).caption);
                        if ((FTests_alcohol.Components[vX] as TMyRadioGroup).visible= true) and
                           ((FTests_alcohol.Components[vX] as TMyRadioGroup).enabled= true) then
                           (FTests_alcohol.Components[vX] as TMyRadioGroup).SetFocus;
                    break;
               End;
        end;
...
procedure TFTests_alcohol.pCalcula(ptag :integer) ;
var
 vX ,
  vz : Integer;
  vcaption :String;
...
Begin
  ...
  for vX :=0 to  FTests_alcohol.ComponentCount -1 do Begin
        if  (FTests_alcohol.Components[vX] is TMyRadioGroup) then begin



                  vcaption     := trim((FTests_alcohol.Components[vX]  as TMyRadioGroup).caption);
                  ...
                  qrespuestas.sql.Text := 'select valor from' +
                                            ...
                                            ' and    id_respuesta= ' + inttostr((FTests_alcohol.Components[vX] as TMyRadioGroup).Itemindex +1);
                 qrespuestas.open;
...
procedure TFTests_alcohol.pLimpiaTest(pTipo :integer);
var
  vX ,
  vz,
  vPuntuacion    : Integer;
...
Begin
 ...
 vBChecadoTotal := False;
  for vX :=0 to  FTests_alcohol.ComponentCount -1 do Begin
        if  (FTests_alcohol.Components[vX] is TMyRadioGroup) Then Begin
                 vBChecadoTotal  :=False;
                 for vz :=0 to  FTests_alcohol.Components[vX].ComponentCount -1 do Begin
                     if  pTipo =1 then
                    (FTests_alcohol.Components[vX].Components[Vz] as TRadioButton).Checked :=False;
                 end;

        end;
         if  (FTests_alcohol.Components[vX] is TMemo) Then
                  if ((pTipo = 1)or(pTipo = 2)) and  (FTests_alcohol.Components[vX].tag <>1) then
                     (FTests_alcohol.Components[vX] as Tmemo).Visible  :=False;

            if  (FTests_alcohol.Components[vX] is TLabel) Then
                  if ((pTipo = 1) or (pTipo = 2)) and  (FTests_alcohol.Components[vX].tag = 1) then
                     (FTests_alcohol.Components[vX] as TLabel).Caption  :='Puntuación | ' ;
 end;
end;

procedure TFTests_alcohol.Button88Click(Sender: TObject);
begin
Close;
end;

procedure TFTests_alcohol.Button93Click(Sender: TObject);
begin
{ DM.Info.First;
 DM.Info.Last;
 no := DM.Info.FieldbyName('no').asInteger;
  inc(no); DM.Info.Appendrecord([no,nil,'']);
 inc(no); DM.Info.Appendrecord([no,nil,'----- TEST DE ALCOHOLISMO ''MALT'' -----']);
 inc(no); DM.Info.Appendrecord([no,nil,'' ]);
If Memo13.Visible = True Then
 inc(no); DM.Info.Appendrecord([no,nil,Memo13.Lines ]);
